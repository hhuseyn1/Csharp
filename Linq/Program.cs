...
class Program
{
    static void Main()
    {
        ...
        // debtors.FindAll(d=>(d.Email.EndsWith("rhyta.com")||(d.Email.EndsWith("dayrep.com")))).Print();


        //3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

        // debtors.FindAll(d=>(d.BirthDay.Year < 1996 && d.BirthDay.Year>1986)).Print();


        //4) Borcu 5000 - den cox olmayan borclularic cixartmag

        //debtors.FindAll(d => (d.Debt <5000)).Print();

        //5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq


        //debtors.FindAll(d => (d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2)).Print();

        //7) Qishda anadan olan borclulari cixardmaq

        //debtors.FindAll(d=>(d.BirthDay.Month==12 || d.BirthDay.Month==1 || d.BirthDay.Month == 2)).Print();
        ...
        //double avgDebt = default;
        //int generalDebt = default;
        //foreach (var debtor in debtors)
        //{
        //    generalDebt += debtor.Debt;
        //}
        //avgDebt = generalDebt / debtors.Count;
        //
        //debtors.FindAll(d=>(d.Debt>avgDebt)).Print();



        //9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq

        //debtors.FindAll(d=>(!d.Phone.Contains("8")));
        //debtors.Select(d => (DateTime.Now.Year - d.BirthDay.Year, d.Debt, d.FullName.Split(" ")[2])).Print();
        ...
        //debtors.FindAll(d=>(d.FullName.Split(" ")[).Print();
        ...
        //debtors.FindAll(d => ((DateTime.Now.Month - d.BirthDay.Month) * 500 > d.Debt) || ( (DateTime.Now.Month - d.BirthDay.Month) * 500 > d.Debt )).Print();

        //20)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
        //debtors.FindAll(d=>(d.FullName.Contains("s") && d.FullName.Contains("m") && d.FullName.Contains("i") && d.FullName.Contains("l") && d.FullName.Contains("e"))).Print();
    }
}